...
export class AdminService {
  ...
  async create(createAdminDto: CreateAdminDto): Promise<interfaceAdmin>{
    ...
    const createAdmin = await new this.adminModel({...createAdminDto,password:hash,refreshToken, role: UserRole.ADMIN});
  ...
  async updateProfile(id: string, updateAdmin: UpdateAdminDto): Promise<interfaceAdmin>{
    if(updateAdmin.password){
      updateAdmin.password = await argon2.hash(updateAdmin.password);
    }
    const update = await this.adminModel.findByIdAndUpdate(id,updateAdmin, {new:true, runValidators: true});
    if(!update){
  ...
  async approvedCompany(companyId: string): Promise<ICompany>{
    const company = await this.companyModel.findByIdAndUpdate(companyId,{status: 'approved'}, {new: true, runValidators: true});
    if(!company){
  ...
  async rejectCompany(companyId : string): Promise<ICompany>{
    const company = await this.companyModel.findByIdAndUpdate(companyId,{status: 'rejected'}, {new: true, runValidators: true});
    if(!company){
      throw new BadRequestException(`company with id ${companyId} not found`);
    }
    return company;
  ...
  async approvedCondidate(condidateId: string): Promise<ICondidate>{
    const condidate = await this.condidateModel.findByIdAndUpdate(condidateId,
      {status: 'approved'},
      {new: true, runValidators: true}
    ); 
    if(!condidate){
      throw new BadRequestException(`condidate with id ${condidateId} not found`)
    }
    return condidate;
  }

  async rejectCondidate(condidateId: string): Promise<ICondidate>{
    const condidate = await this.condidateModel.findByIdAndUpdate(condidateId,{status: 'rejected'}, {new: true, runValidators: true})
    if (!condidate){
    throw  new BadRequestException(`condidate with id ${condidateId} not found`);
    }
    return condidate;
  ...
  async getCondidateByID(condidateId: string): Promise<ICondidate>{
    const condidate = await this.condidateModel.findById(condidateId).populate('jobApplications').populate('testJobApplication');
    if(!condidate){
      throw new BadRequestException(`condidate with id ${condidateId} not found`)
    }
    return condidate;
  ...
  async getCompanyById(companyId: string): Promise<ICompany>{
    const company = await this.companyModel.findById(companyId).populate('jobApplications').populate('testJobApplication');
    if(!company){
      throw new BadRequestException(`company with id ${companyId} not found`)
    }
    return company;
  ...
  async approveJobOffer(jobOfferId: string): Promise<IJobOffer>{
    const updatedJobOffer = await this.jobOfferService.update(jobOfferId, {status: 'approved'});
    if(!updatedJobOffer){
  ...
  async rejectJobOffer(jobOfferId: string): Promise<IJobOffer>{
    const updatedJobOffer = await this.jobOfferService.update(jobOfferId, {status: 'rejected'})
    if(!updatedJobOffer){
      throw new NotFoundException(`job Offfer with id ${jobOfferId} not found`);
    }
    return updatedJobOffer;
  }
  async approveJobApplication(jobApplicationId: string): Promise<IJobApplication>{
    const updatedApplication = await this.jobApplicationService.updateStatus(jobApplicationId, ApplicationStatus.Approved);
    if(!updatedApplication){
      throw new NotFoundException(`job Application with Id ${jobApplicationId} not found`)
    }
    return updatedApplication;
  }
  async rejectApplication(jobApplicationId: string): Promise<IJobApplication>{
    const updatedApplication = await this.jobApplicationService.update(jobApplicationId, {status: ApplicationStatus.Rejected});
    if(!updatedApplication){
      throw new NotFoundException(` job application with id  ${jobApplicationId} not found`);
    }
    return updatedApplication;
