...
export class CommentService {
 ...
 async findAll(companyId: string): Promise<Comment[]>{
  return this.commentModel.find({company: companyId}).populate('condidate' , 'name').exec();
 }
 async findByJobOffer(jobOfferId: string): Promise<Comment[]>{
    return  this.commentModel.find({jobOffer: jobOfferId}).populate('condidate', 'name').populate('company', 'name').exec();
 }
 async respondToComment(
    ...
    if (role === 'company') {
      const jobOffer = await this.jobOfferModel.findOne({
 ...
 async moderateComment(commentId: string, moderatDto: ModerateCommentDto): Promise<Comment>{
    const updatedComment = await this.commentModel.findByIdAndUpdate(commentId, {status: moderatDto.status},{new: true},).exec();
if(!updatedComment){
...
async delete(commentId: string): Promise<void>{
 ...
 async GetAllComments(): Promise<Comment[]>{
    const getallComments = await this.commentModel.find().populate('condidate', 'name');
 ...
 async getRecentComments(limit: number): Promise<Comment[]> {
    return this.commentModel
      ...
      .select('content condidate company jobOffer status createdAt updatedAt companyResponse condidateResponse')
      .exec();
  }
