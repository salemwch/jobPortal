import { Controller, Get, Post, Body, Patch, Param, Delete, HttpStatus, Res, UploadedFile , UseInterceptors, Put, Req, UseGuards, BadRequestException, Query} from '@nestjs/common';
...
export class CompanyController {
  ...
  async register(@Res() response, @Body() createCompanyDto: CreateCompanyDto,@UploadedFile() file ){
    try{
      if(file){
  ...
  async update(@Res() response, @Param('id') id: string, @Body() updateCompanyDto: UpdateCompanyDto, @UploadedFile() file){
    try{
      if(file && file.name){
  ...
  @Patch(':id/desiredFields')
    async updateDesiredFields(@Param('id') id: string, @Body() updatecompanyDto: UpdateCompanyDto){
      try{
        const updateCompany = await this.companyService.updateDesiredFields(id, updatecompanyDto);
