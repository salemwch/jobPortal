...
export class CondidatesService {
  ...
  async create(createCondidateDto: CreateCondidateDto): Promise<ICondidate>{
    const existingCondidate = await this.condidateModel.findOne({email: createCondidateDto.email});
    if(existingCondidate){
      throw new BadRequestException(`condidate with email ${createCondidateDto.email} already exist`)
    }
  ...
  async getCondidateByID(condidateId: string){
    return this.condidateModel.findById(condidateId).populate('jobApplications').exec();
  }
  
  async findByEmail(email: string): Promise<ICondidate>{
    const condidate = await this.condidateModel.findOne({email, role: UserRole.CONDIDATE})
    if(!condidate){
      throw new BadRequestException(`condidate with email : ${email} not found`)
    }
    return condidate;
  }
  async updateProfile(id: string, updateData: UpdateCondidateDto): Promise<ICondidate>{
    if(updateData.password){
      updateData.password = await argon2.hash(updateData.password)
    }
        const update = await  this.condidateModel.findByIdAndUpdate(id, {$set: updateData},{new: true, runValidators: true}).lean();
        if(!update){
  ...
  async ListAllCondidates(): Promise<ICondidate[]>{
    const condidates = await this.condidateModel.find({role: UserRole.CONDIDATE});
 ...
 async updateDesiredFields(id: string, updateDto: UpdateCondidateDto):Promise<ICondidate>{
  const condidate = await this.condidateModel.findByIdAndUpdate(id, updateDto, {new:true});
  if(!condidate){
    throw new BadRequestException(`condidate with id ${id} not found` )
  }
  return condidate;
 ...
 async incrementViewCount(id: string): Promise<ICondidate>{
  ...
  return this.condidateModel.findByIdAndUpdate(id, {$inc: { viewCount: 1}}, {new: true});
 }
