...
export class EvaluationController {
  ...
  @Post(':companyId')
  async createEvaluation(@Res() response, @Param('companyId') companyId: string, @Body() {condidateId, rating,review}){
    try{
      const evaluation = await this.evaluationService.createEvaluation(condidateId,companyId,rating,review);
  ...
  async getEvaluation(@Res() response, @Param('companyId') companyId: string){
    try{
      const evalutions = await this.evaluationService.getEvaluationForCompany(companyId);
  ...
  @Delete(':companyId')
      async deleteEvaluation(@Res() response, @Param('companyId')  companyId: string, @Body() {condidateId}){
        try{
  ...
  try {
    const evaluation = await this.evaluationService.getEvaluationByCondidate(condidateId, companyId);
    return response.status(HttpStatus.OK).json({
