...
export class JobApplicationService {
 constructor(
  @InjectModel('JobApplication') private readonly jobApplicationModel: Model<IJobApplication>,
 ...
 async create(createJobApplication: CreateJobApplicationDto): Promise<IJobApplication> {
  ...
  const existingApplication = await this.jobApplicationModel.findOne({
      ...
      testJobApplication: new Types.ObjectId(createJobApplication.testJobApplication),
  });
  if (existingApplication) {
      throw new BadRequestException('This condidate has already applied for this job');
  }

  const jobTest = await this.testJobApplicationService.findOnetest(createJobApplication.testJobApplication);
  if (!jobTest) {
  ...
  if (!jobTest.jobOffer.equals(jobOfferId)) {
      throw new BadRequestException('TestJobApplication does not belong to the specified job offer');
  }
  ...
  const jobOffer = await this.jobOfferModel.findById(createJobApplication.jobOffer).exec();
  if (jobOffer) {
      await this.notificationService.create({
          message: `A new condidate has applied for your job offer: ${jobOffer.title}`,
          user: isObjectId(jobOffer.company) ? jobOffer.company.toString() : jobOffer.company,
          type: "jobApplication",
      });
  }
...
async createForJob(jobOfferId: string, createJobApplicationDto: CreateJobApplicationDto) {
  console.log(`Applying for job offer with ID: ${jobOfferId}`, createJobApplicationDto);
  return { message: `Application for job offer ${jobOfferId} submitted successfully!` };
}
 ...
 async findOneJob(id: string): Promise<IJobApplication>{
  ...
  if(!findOneJobApplication){
    throw new BadRequestException(`job application with thd id ${id} not found`);
  }
  return findOneJobApplication;
 }
 async update(id: string, updateJobApplication: UpdateJobApplicationDto){
  const updatedJobApplication = await this.jobApplicationModel.findByIdAndUpdate(id,updateJobApplication, {new: true});
  if(!updatedJobApplication){
    throw new NotFoundException(` job application with the id ${id} not found`);
  }
  return updatedJobApplication;
 }
 async delete(id: string): Promise<IJobApplication>{
  const deletedJobApplication = await this.jobApplicationModel.findByIdAndDelete(id);
  if(!deletedJobApplication){
    throw new BadRequestException(`job application with the id ${id} not found`);
  }
  return deletedJobApplication;
 }
 async evaluateTestResult(applicationId: string, testResultDto:{questionId: string; answer: string}): Promise<IJobApplication>{
  ...
  const test = application.testResults.find((t) => t.questionId === testResultDto.questionId);
  if(!test){
    throw new NotFoundException(`test with Id ${testResultDto.questionId} not found in application`);
  }
  if(test.answer  && test.isCorrect !== null ){
    throw new BadGatewayException('this test has already been evaluated');
  }
  const isCorrect = await this.testJobApplicationService.evaluateAnswer(testResultDto.questionId, testResultDto.answer);
  ...
  async submitfinaltestResult(applicationId: string): Promise<{status: string; score: number}>{
  const applicationn = await this.jobApplicationModel.findById(applicationId);
  if(!applicationn){
    throw new NotFoundException(`job applciation with id ${applicationId} not found`);
  }
  const unansweredTests = applicationn.testResults.filter((t) => t.answer === null);
  ...
  const correctAnswer = applicationn.testResults.filter((t)=> t.isCorrect === true).length;
  ...
  await this.testJobApplicationModel.findByIdAndUpdate(applicationn.testJobApplication,
  ...
  async updateStatus(applicationId: string, status: string): Promise<IJobApplication>{
