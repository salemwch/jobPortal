...
export class JobOfferController {
  ...
  async create(@Res() response, @Body() createJobOfferdto: CreateJobOfferDto){
    try{
      const createJobOffer = await this.jobOfferService.create(createJobOfferdto);
  ...
  async findTestsByJobOfferId(@Param('jobOfferId') jobOfferId: string) {
    try {
      const tests = await this.jobOfferService.findTestsByJobOfferId(jobOfferId);
      return { tests };
    } catch (error) {
  ...
  @Put(':id')
  async update(@Res() response, @Param('id') id: string, @Body() updateJobOfferDto: UpdateJobOfferDto){
    try{
      const updateJobOffer = await this.jobOfferService.update(id, updateJobOfferDto);
  ...
  @Delete(':id')
    ...
    async findMostViewd(@Res() response, @Query('limit') limit: number){
      try{
        const mostViewdOffers = await this.jobOfferService.findMostViewd(limit || 5);
        return response.status(HttpStatus.CREATED).json({
    ...
    async updateStatus(@Param('id') id:string, @Body() updateStatusOffer: UpdateJobOfferDto){
      try{
        const updatedJobOfferStatus = await this.jobOfferService.updatedStatusOffer(id, updateStatusOffer);
        return {message: 'jobOffer status fields updated', updatedJobOfferStatus}
      }catch(error){
        return {message: `error updating jobOFfer status`, error: (error as Error).message}
      }
    }
    @Get('/company/:companyId')
async getJobOffersByCompany(@Res() response, @Param('companyId') companyId: string) {
  try{
    const getJobOffersByCompanyId = await this.jobOfferService.findCompanybyID(companyId);
    return response.status(HttpStatus.OK).json({
