import { Controller, Get, Post, Body, Patch, Param, Delete, Res, HttpStatus, Query, UseGuards, NotFoundException } from '@nestjs/common';
...
export class TestJobApplicationController {
  constructor(private readonly testJobApplicationService: TestJobApplicationService) {}
  @UseGuards(RefreshTokenGuards, RoleGuard)
  @Role(UserRole.COMPANY)
  @Post()
  async create(@Res() response, @Body() createJobTestDto: CreateTestJobApplicationDto){
   try{
    const newJobTest = await this.testJobApplicationService.create(createJobTestDto);
  ...
  async findOne(@Res() response, @Param('id') id: string){
    try{
      const getJobtestByid = await this.testJobApplicationService.findOnetest(id);
  ...
  async evaluateAnswer(@Res() response, @Query('questionId') questionId: string, @Query('answer') answer: string){
    try{
      const IsCorrect = await this.testJobApplicationService.evaluateAnswer(questionId, answer);
      return response.status(HttpStatus.OK).json({
  ...
  async update(@Res() response, @Param('id') id: string, @Body() updatetestjobApplication: UpdateTestJobApplicationDto){
    try{
      const updatedJobTest = await this.testJobApplicationService.update(id, updatetestjobApplication);
      return response.status(HttpStatus.ACCEPTED).json({
  ...
  @Get(':jobOffer/tests')
async findtestByJobOfferId(@Res() response, @Param('jobOffer') jobOfferId: string) {
  try {
    const tests = await this.testJobApplicationService.findTestsByJobOfferId(jobOfferId);
    return response.status(HttpStatus.OK).json({
